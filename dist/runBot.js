...
exports.runBot = ({ bot, st }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    console.log(`${helpers_1.dateit()} ${EXCHANGE} initialized`);
    // get balances
    ...
    console.log(`${helpers_1.dateit()} ${EXCHANGE}:: ${amountCoin1} ${COIN1}`);
    console.log(`${helpers_1.dateit()} ${EXCHANGE}:: ${amountCoin2} ${COIN2}`);
    console.log(`${helpers_1.dateit()} ${EXCHANGE}:: ${price} ${pair}`);
    ...
    if (isBuy) {
        ...
        if (isEnoughAvailable) {
            ...
            console.log(`${helpers_1.dateit()} ${EXCHANGE}:: Placing market buy for`, orderSize, COIN1, 'with', COIN2);
            try {
                ...
                console.log(`${helpers_1.dateit()} ${EXCHANGE}:: Done. Bought ${res.filled} ${COIN1} for ${res.cost} ${COIN2} at ${res.average} ${res.symbol} paying ${res
                    .fee.cost} ${res.fee.currency} fee.`);
            }
            catch (e) {
                console.warn(`${helpers_1.dateit()} ${EXCHANGE}:: ${e.name}`);
            }
        }
        else {
            console.log(`${helpers_1.dateit()} ${EXCHANGE}:: min trade amount is ${minTradeUnit2} ${COIN2} but only ${amountCoin2} ${COIN2} available `);
        }
    }
    /* --------------------------------- selling -------------------------------- */
    if (isSell) {
        ...
        if (isEnoughAvailable) {
            ...
            console.log(`${helpers_1.dateit()} ${EXCHANGE}:: Placing market sell for`, pair, 'in amount of', orderSize, COIN1);
            try {
                const res = yield thisExchange.createMarketSellOrder(pair, orderSize);
                // console.log('res:', JSON.stringify(res, null, 2))
                console.log(`${helpers_1.dateit()} ${EXCHANGE}:: Done. Sold ${res.filled} ${COIN1} for ${res.cost} ${COIN2} at ${res.average} ${res.symbol} paying ${res
                    .fee.cost} ${res.fee.currency} fee.`);
            }
            catch (e) {
                console.warn(`${helpers_1.dateit()} ${EXCHANGE}:: ${e.name}`);
            ...
            console.log(`${helpers_1.dateit()} ${EXCHANGE}:: min trade amount is ${minTradeUnit1} ${COIN1} but only ${amountCoin1} ${COIN1} available `);
        }
    }
});
